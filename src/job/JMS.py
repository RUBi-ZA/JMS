...
class JMS:
    ...
    def GetClusterJob(self, job_id, username, password):
        ...
        try:
            ...
            job = ClusterJob()
            job.ClusterJobID = j.get("ClusterJobID", None)
            job.JobName = j.get("JobName", None)
            job.JobOwner = j.get("JobOwner", None)
    
            job.MemoryRequested = j.get("MemoryRequested", None)
            job.NodesAvailable = j.get("NodesAvailable", None)
            job.NodesRequested = j.get("NodesRequested", None)
            job.WalltimeRequested = j.get("WalltimeRequested", None)
    
            job.CPUTimeUsed = j.get("CPUTimeUsed", None)
            job.MemoryUsed = j.get("MemoryUsed", None)
            job.VirtualMemoryUsed = j.get("VirtualMemoryUsed", None)
            job.WalltimeUsed = j.get("WalltimeUsed", None)
    
            job.State = j.get("State", None)
            job.Queue = j.get("Queue", None)
            job.Server = j.get("Server", None)
            job.ExecutionHost = j.get("ExecutionHost", None)
            job.SubmitArgs = j.get("SubmitArgs", None)
            job.SubmitHost = j.get("SubmitHost", None)
            job.OutputPath = j.get("OutputPath", None)
            job.ErrorPath = j.get("ErrorPath", None)
            job.Priority = j.get("Priority", 0)
    
            job.CreatedTime = j.get("CreatedTime", None)
            job.TimeEnteredQueue = j.get("TimeEnteredQueue", None)
            job.EligibleTime = j.get("EligibleTime", None)
            job.LastModified = j.get("LastModified", None)
            job.StartTime = j.get("StartTime", None)
            job.CompletionTime = j.get("CompletionTime", None)
            job.TotalRuntime = j.get("TotalRuntime", None)
    
            job.VariableList = j.get("VariableList", None)
    
            job.Comment = j.get("Comment", None)
            job.ExitStatus = j.get("ExitStatus", None)
    ...
    def UpdateJobState(self, ClusterJobID, StatusID):
        ...
        #Create snapshot of job at this point in time
        user = jobstage.Job.User
